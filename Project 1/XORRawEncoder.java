...
public class XORRawEncoder extends RawErasureEncoder {
  ...
  protected void doEncode(ByteBufferEncodingState encodingState) {
	...
	for (iIdx = encodingState.inputs[0].position(), oIdx = output.position(); iIdx < encodingState.inputs[0].limit(); iIdx++, oIdx++)
	{
		output.put(oIdx, (byte) (encodingState.inputs[0].get(iIdx) ^ encodingState.inputs[1].get(iIdx)))；
	}
  }
  ...
  protected void doEncode(ByteArrayEncodingState encodingState) {
	...
	{
      output[oIdx] = encodingState.inputs[0][iIdx] ^ encodingState.inputs[1][iIdx];
    }
  }
}
